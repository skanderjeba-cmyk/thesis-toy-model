...
we ALSO emit scenario-named “canonical” figures so downstream selection reports
“already named” for all of them:

  - figures/fig_wage_ratio_<SCENARIO_SLUG>.png
  - figures/fig_migration_diagnostics_<SCENARIO_SLUG>.png
  - figures/fig_capital_dilution_<SCENARIO_SLUG>.png     (optional tidy)
  - figures/fig_emissions_ratio_<SCENARIO_SLUG>.png      (optional tidy)
...
def main() -> int:
    ...
    def log(msg: str):
        # Avoid non-ASCII to keep Windows codepages happy
    ...
    try:
        ...
        if scen_slug:
            ...
            try:
                ...
                plt.ylabel("$\\rho_t$")
                _ensure_dir(figures_dir)
                plt.savefig(os.path.join(figures_dir, f"fig_wage_ratio_{scen_slug}.png"),
            ...
            # --- Migration diagnostics saved under the SCENARIO slug ---
            try:
                rec_diag = raw.get("ease_migration") or raw.get("ease_migration_strong")
